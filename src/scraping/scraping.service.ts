...
export class ScrapingService {
  ...
  private async extractAndSaveData(page: puppeteer.Page) {
    const maxPageNumber = await page.evaluate(() => {
      const pageLinks = Array.from(
        document.querySelectorAll('.pagination .page-link[data-page]'),
      );
      const pageNumbers = pageLinks.map((link) =>
        parseInt(link.getAttribute('data-page') || '0', 10),
      );
  ...
  private async reloadAndNavigateToPage(
    page: puppeteer.Page,
    currentPage: number,
  ) {
    ...
    for (let i = 1; i < rows.length; i++) {
      ...
      if (row.length > 1) {
        ...
        try {
          ...
          console.log(
            `Saved: ${scrapingEntity.razonSocial}, ${scrapingEntity.numRegistro}`,
          );
        } catch (error) {
  ...
  async scrapeData(url: string): Promise<any> {
    ...
    await new Promise((resolve) => setTimeout(resolve, 4000));
    await page.waitForSelector('#arrowdiv-consulta');
        ...
        for (let k = 0; k < letters.length; k++) {
          ...
          while (retry) {
            ...
            const captchaError = await page.evaluate(() => {
              ...
              return errorElement
                ? errorElement.textContent.includes('Validation capcha error.')
                : false;
            });
